...
DEFAULT_COLLECTION = 'dabac_policies'
DEFAULT_MIGRATION_COLLECTION = 'dabac_policies_migration_version'
    ...
    def down(self):
        def process(doc):
            ...
            for name, rule in doc['rules'].items():
                ...
                if not rule_type.startswith('dabac.rules.'):
                ...
                # dabac's own RegexMatchRule couldn't be stored in mongo because is has non-primitive data,
                # so it's impossible to put it to storage if we revert time back to 1.1.0
                elif rule_type == 'dabac.rules.string.RegexMatchRule':
                    raise Irreversible('dabac.rules.string.RegexMatchRule could not be stored in v1.1.0')
...
class Migration1x1x1To1x2x0(MongoMigration):
    ...
    def __init__(self, storage):
        ...
        self.rules_rename = {
            'dabac.rules.string.StringEqualRule': 'dabac.rules.string.Equal',
            'dabac.rules.string.RegexMatchRule': 'dabac.rules.string.RegexMatch',
            'dabac.rules.string.StringPairsEqualRule': 'dabac.rules.string.PairsEqual',
            'dabac.rules.net.CIDRRule': 'dabac.rules.net.CIDR',
            'dabac.rules.inquiry.SubjectEqualRule': 'dabac.rules.inquiry.SubjectEqual',
            'dabac.rules.inquiry.ActionEqualRule': 'dabac.rules.inquiry.ActionEqual',
            'dabac.rules.inquiry.ResourceInRule': 'dabac.rules.inquiry.ResourceIn',
        }
            ...
            for rule in doc['context'].values():
                ...
                for old, new in self.rules_rename.items():
                    if rule_type == new:
                        ...
                        break
                if rule_type.startswith('dabac.rules.list') or \
                        rule_type.startswith('dabac.rules.logic') or \
                        rule_type.startswith('dabac.rules.operator') or \
                        rule_type in ['dabac.rules.string.StartsWith',
                                      'dabac.rules.string.EndsWith',
                                      'dabac.rules.string.Contains']:
