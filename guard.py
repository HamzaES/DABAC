""""
Main module that serves as an entry point for dabac decisions.
Also contains Inquiry class. and feedback factor from the Outlier Detection Component
"""

import logging
from OutlierML.Detector import detectOutlierIF, detectOutlierLOF
...
class Guard:
    ...
    def check_context_restriction(policy, inquiry):
        ...
        Check if the request is anomalous, if it is -> deny access.
        You can choose between IsolationForest and LocalOutlierFactor
        """
        for key, rule in policy.context.items():
            ...
            if not rule.satisfied(ctx_value, inquiry):
                return False
            if round(detectOutlierIF(inquiry)):
                return False
            """
            uncomment to use LocalOutlierFactor and comment the two lines above to disable IsolationForest
            if round(detectOutlierLOF(inquiry)):
                return False
            """
        return True

    def check_context_restriction(inquiry):
        """
        Check if the request is anomalous, if it is -> deny access.
        You can choose between IsolationForest and LocalOutlierFactor
        """
        if detectOutlierIF(inquiry):
            return False
